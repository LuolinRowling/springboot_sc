...
jwplayer.version = "6.7.4071", jwplayer.vid = document.createElement("video"), jwplayer.audio = document.createElement("audio"), jwplayer.source = document.createElement("source"),
function(e) {
	function a(b) {
		return function() {
			return c(b)
	...
	function f(m, a, c, g) {
		return function() {
			...
			} catch(f) {} (d = b.parseXML(m.responseText)) && d.firstChild ? (m = b.extend({},
			m, {
				responseXML: d
	...
	b.getCookies = function() {
		for (var b = {},
		a = j.cookie.split("; "), d = 0; d < a.length; d++) {
			var g = a[d].split("\x3d");
	...
	b.deepReplaceKeyName = function(a, c, d) {
		...
		case "object":
			...
			function(b, g) {
				...
				for (var h = b,
				j = 0; j < f.length; j++) h = h.replace(RegExp(c[j], "g"), d[j]);
	...
	b.ajax = function(a, c, h) {
		...
		if (j && b.exists(d.XDomainRequest)) g = new XDomainRequest,
		g.onload = f(g, a, c, h),
		g.onerror = k(h, a, g),
	...
	b.filterPlaylist = function(a, c) {
		...
		if (c && 0 == d.length) for (g = 0; g < a.length; g++) if (f = b.extend({},
		a[g]), f.sources = b.filterSources(f.sources, !0), 0 < f.sources.length) {
			for (h = 0; h < f.sources.length; h++) n = f.sources[h],
			n.label || (n.label = h.toString());
			d.push(f)
		}
		return d
	...
	b.filterSources = function(a, c) {
		...
		if (a) {
			...
			for (var h = 0; h < a.length; h++) {
				...
				c ? e.embed.flashCanPlay(j, n) && (d || (d = n), n == d && g.push(b.extend({},
				a[h]))) : b.canPlayHTML5(n) && (d || (d = n), n == d && g.push(b.extend({},
				a[h])))
			}
		}
		return g
...
function(e) {
	...
	e.scriptloader = function(f) {
		...
		this.load = function() {
			...
			if (c && (c.getStatus() == a.NEW || c.getStatus() == a.LOADING)) c.addEventListener(b.ERROR, j),
			c.addEventListener(b.COMPLETE, d);
			else if (e.scriptloader.loaders[f] = this, h == a.NEW) {
				h = a.LOADING;
				var m = k.createElement("script");
				m.addEventListener ? (m.onload = d, m.onerror = j) : m.readyState && (m.onreadystatechange = function() { ("loaded" == m.readyState || "complete" == m.readyState) && d()
				});
...
function(e) {
	...
	e.touch = function(f) {
		...
		function d(a, b, c) {
			if (q[a] && (b.preventManipulation && b.preventManipulation(), b.preventDefault && b.preventDefault(), b = c ? c: h(a, b))) q[a](b)
		}
		function h(a, c) {
			var d = null;
...
function(e) {
	e.key = function(a) {
		...
		this.edition = function() {
			return j && j.getTime() < (new Date).getTime() ? "invalid": k
		};
		this.token = function() {
			return f
		};
		...
		try {
			...
			var d = a.split("/"); (k = d[0]) ? /^(free|pro|premium|ads)$/i.test(k) ? (f = d[1], d[2] && 0 < parseInt(d[2]) && (j = new Date, j.setTime(String(d[2])))) : k = "invalid": k = "free"
		} catch(h) {
			k = "invalid"
		}
	}
...
function(e) {
	...
	a.encrypt = function(j, d) {
		...
		1 >= h.length && (h[1] = 0);
		for (var b = a.strToLongs(f.encode(d).slice(0, 16)), c = h.length, e = h[c - 1], m = h[0], p, l = Math.floor(6 + 52 / c), g = 0; 0 < l--;) {
			g += 2654435769;
			p = g >>> 2 & 3;
			for (var r = 0; r < c; r++) m = h[(r + 1) % c],
			e = (e >>> 5 ^ m << 2) + (m >>> 3 ^ e << 4) ^ (g ^ m) + (b[r & 3 ^ p] ^ e),
			e = h[r] += e
		}
		h = a.longsToStr(h);
		return k.encode(h)
	...
	a.decrypt = function(j, d) {
		...
		for (var h = a.strToLongs(k.decode(j)), b = a.strToLongs(f.encode(d).slice(0, 16)), c = h.length, e = h[c - 1], m = h[0], p, l = 2654435769 * Math.floor(6 + 52 / c); 0 != l;) {
			p = l >>> 2 & 3;
			for (var g = c - 1; 0 <= g; g--) e = h[0 < g ? g - 1 : c - 1],
			e = (e >>> 5 ^ m << 2) + (m >>> 3 ^ e << 4) ^ (l ^ m) + (b[g & 3 ^ p] ^ e),
			m = h[g] -= e;
			l -= 2654435769
		}
		h = a.longsToStr(h);
	...
	var k = {
		...
		encode: function(a, d) {
			...
			l = g.length % 3;
			if (0 < l) for (; 3 > l++;) p += "\x3d",
			g += "\x00";
			for (l = 0; l < g.length; l += 3) h = g.charCodeAt(l),
			b = g.charCodeAt(l + 1),
			c = g.charCodeAt(l + 2),
			e = h << 16 | b << 8 | c,
			h = e >> 18 & 63,
			...
			c = e >> 6 & 63,
			e &= 63,
			m[l / 3] = r.charAt(h) + r.charAt(b) + r.charAt(c) + r.charAt(e);
			m = m.join("");
		...
		decode: function(a, d) {
			...
			g = k.code;
			l = d ? f.decode(a) : a;
			for (var r = 0; r < l.length; r += 4) h = g.indexOf(l.charAt(r)),
			b = g.indexOf(l.charAt(r + 1)),
			e = g.indexOf(l.charAt(r + 2)),
			m = g.indexOf(l.charAt(r + 3)),
			c = h << 18 | b << 12 | e << 6 | m,
			h = c >>> 16 & 255,
			b = c >>> 8 & 255,
			c &= 255,
			p[r / 4] = String.fromCharCode(h, b, c),
			64 == m && (p[r / 4] = String.fromCharCode(h, b)),
			64 == e && (p[r / 4] = String.fromCharCode(h));
			e = p.join("");
...
function(e) {
	...
	e.eventdispatcher = function(e, f) {
		...
		this.removeGlobalListener = function(f) {
			if (f) {
				try {
					for (var b = 0; b < d.length; b++) if (d[b].listener.toString() == f.toString()) {
						d.splice(b, 1);
						break
					}
				} catch(c) {
		...
		this.sendEvent = function(h, b) {
			...
			for (c = 0; c < d.length; c++) {
				try {
					...
					a.log("There was an error while handling a listener: " + m.toString(), d[c].listener)
				}
...
function(e) {
	...
	e.plugin = function(f) {
		...
		a.extend(this, l);
		this.load = function() {
			if (b == a.loaderstatus.NEW) if (0 < f.lastIndexOf(".swf")) c = f,
			b = a.loaderstatus.COMPLETE,
			l.sendEvent(k.COMPLETE);
	...
	e.plugins.pluginloader = function(j, d) {
		...
		function b() {
			...
			if (!q && !m) {
				...
				function(d) {
					...
					else if (d && (!g || parseFloat(g) > parseFloat(e.version))) m = !0,
		...
		this.setupPlugins = function(b, c, d) {
			...
			function(f, j) {
				...
				r = l.getJS(),
				p = l.getURL();
				q && (g.plugins[q] = a.extend({},
				j), g.plugins[q].pluginmode = l.getPluginmode(), g.length++);
				try {
					if (r && c.plugins && c.plugins[p]) {
						...
						A.style.top = 0;
						A.style.zIndex = e + 10;
						h[k] = l.getNewInstance(b, a.extend({},
						c.plugins[p]), A);
						e++;
						b.onReady(d(h[k], A, !0));
						b.onResize(d(h[k], A))
...
function(e) {
	...
	h.parseGroup = function(b, c) {
		...
		case "content":
			k(e, "duration") && (c.duration = a.seconds(k(e, "duration")));
...
function(e) {
	...
	e.rssparser.parse = function(b) {
		for (var c = [], f = 0; f < d(b); f++) {
			var e = j(b, f);
			if ("channel" == h(e).toLowerCase()) for (var k = 0; k < d(e); k++) {
				var l = j(e, k);
...
function(e) {
	e.playlist = function(a) {
		var k = [];
		if ("array" == e.utils.typeOf(a)) for (var f = 0; f < a.length; f++) k.push(new e.playlist.item(a[f]));
...
function(e) {
	...
	e.loader = function() {
		function a(d) {
			try {
				var h = d.responseXML.childNodes;
				d = "";
...
function(e) {
	...
	b = e.embed = function(c) {
		...
		function m() {
			if (!D) if ("array" == k.typeOf(n.playlist) && 2 > n.playlist.length && (0 == n.playlist.length || !n.playlist[0].sources || 0 == n.playlist[0].sources.length)) g();
			else if (!C) if ("string" == k.typeOf(n.playlist)) {
				...
				for (a = 0; a < n.modes.length; a++) if (n.modes[a].type && b[n.modes[a].type]) {
					...
					if (l.supportsConfig()) return l.addEventListener(f.ERROR, p),
					l.embed(),
					q(c, h.events),
					c
				}
		...
		w = e.plugins.loadPlugins(c.id, n.plugins),
		C = d,
		D = d,
		E = null;
...
function(e) {
	function a(a) {
		if (a.playlist) for (var e = 0; e < a.playlist.length; e++) a.playlist[e] = new j(a.playlist[e]);
		else {
			var b = {};
			f.foreach(j.defaults,
			function(c) {
				k(a, b, c)
			});
			b.sources || (a.levels ? (b.sources = a.levels, delete a.levels) : (e = {},
			k(a, e, "file"), k(a, e, "type"), b.sources = e.file ? [e] : []));
			a.playlist = [new j(b)]
		}
	}
	function k(a, e, b) {
	...
	var f = e.utils,
	j = e.playlist.item; (e.embed.config = function(d) {
		var h = {
			fallback: !0,
			height: 270,
			primary: "html5",
			width: 480,
			base: d.base ? d.base: f.getScriptPath("jwplayer.js"),
			aspectratio: ""
		};
		...
		if ("string" != typeof b || !f.exists(b)) h = 0;
			...
			var c = b.indexOf(":"); - 1 == c ? h = 0 : (h = parseFloat(b.substr(0, c)), b = parseFloat(b.substr(c + 1)), h = 0 >= h || 0 >= b ? 0 : 100 * (b / h) + "%")
...
function(e) {
	...
	e.embed.download = function(f, e, d) {
		function h(b, c) {
			for (var d = k.querySelectorAll(b), f = 0; f < d.length; f++) a.foreach(c,
			function(a, b) {
				d[f].style[a] = b
			})
		}
		...
		if (c && c.length) {
			...
			l ? (p = l, d = g, f && (c = b("a", "display", f), b("div", "icon", c), b("div", "logo", c), p && c.setAttribute("href", a.getAbsolutePath(p))), c = "#" + f.id + " .jwdownload", f.style.width = "", f.style.height = "", h(c + "display", {
...
function(e) {
	var a = e.utils,
	k = e.events,
	f = {}; (e.embed.flash = function(d, h, b, c, q) {
		function m(a, b, c) {
...
function(e) {
	...
	e.embed = k.extend(function(f) {
		...
		n,
		q = q ? q: {};
		"ads" == u && b.advertising && (b.advertising.client.match(".js$|.swf$") ? q[b.advertising.client] = b.advertising: q[d + b.advertising.client + ".js"] = b.advertising);
		...
		case "premium":
			b.related && (b.related.client && b.related.client.match(".js$|.swf$") && (g = b.related.client), q[g] = b.related),
			b.ga && (b.ga.client && b.ga.client.match(".js$|.swf$") && (r = b.ga.client), q[r] = b.ga);
		case "pro":
			b.sharing && (b.sharing.client && b.sharing.client.match(".js$|.swf$") && (l = b.sharing.client), q[l] = b.sharing),
...
function(e) {
	...
	h = e.api = function(a) {
		...
		function q(a, b) {
			u[a] || (u[a] = [], p(f.JWPLAYER_PLAYER_STATE,
			function(b) {
				var c = b.newstate;
				b = b.oldstate;
				if (c == a) {
		...
		g.pause = function(a) {
			"undefined" == typeof a ? (a = g.getState(), a == j.PLAYING || a == j.BUFFERING ? l("jwPause") : l("jwPlay")) : l("jwPause", a);
			return g
		};
		g.stop = function() {
			l("jwStop");
		...
		g.playerReady = function(a) {
			...
			p(f.JWPLAYER_PLAYLIST_ITEM,
			function() {
				y = {}
			});
			p(f.JWPLAYER_MEDIA_META,
			function(a) {
...
function(e) {
	...
	e.api.instream = function(e, d, h, b) {
		...
		function q(b, e) {
			g[b] || (g[b] = [], c(a.JWPLAYER_PLAYER_STATE,
			function(a) {
				var c = a.newstate,
				e = a.oldstate;
				if (c == b) {
		...
		this.updateSkipTime = function(a, b) {
